...
def visualize_results(image_path, pred_mask, true_mask, pred_landmarks, true_landmarks, save_path):
  ...
  for j, landmark in enumerate(true_landmarks):
      x, y = landmark
      if x != -100 and y != -100:
          draw.ellipse([(x*1280 - 10, y*720 - 10), (x*1280 + 10, y*720 + 10)], fill=cls_colors[j+2])
...
def evaluate_model(pred_seg_folder, true_seg_folder, pred_keypoints_file, true_keypoints_file, image_folder, output_folder):
  ...
  for _, row in pred_df.iterrows():
      ...
      save_path = os.path.join(output_folder, f"pseudo_vs_true_{image_name}")
  ...
  return iou, precision, avg_keypoint_distance, mpck
...
if __name__ == "__main__":
  ...
  output_folder = '/home/zhehua/data/PitDatasets/pseudo_vs_true_visualization'  

  if not os.path.exists(output_folder):
      os.makedirs(output_folder)

  iou, precision, avg_keypoint_distance, mpck = evaluate_model(
