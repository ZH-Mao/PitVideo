...
from lib.core.function_video_nMasks_sstan import train, validate, test
from lib.datasets.pitVideoDataset_3Masks import PitDataset
from lib.models.segland_hrnet_SSSTAN import HRNET_SSTAN
import random
from lib.core import mmwing_loss, focal_loss, sstan_loss
...
def parse_args():
    ...
    #                     type=str)
    parser.add_argument('--cfg',
                        default=r'/workspace/projects/PitVideo-Segment-Landmark/experiments/pituitary/video_hrnet_ssstan_w48_2stage_fold3.yaml',
                        help='experiment configure file name',
                        type=str)
...
def main():
    ...
    # build model
    model = HRNET_SSTAN(config)
    model.init_weights(config.MODEL.PRETRAINED)              
    model = model.to(device)
    model = torch.nn.DataParallel(model)

    # # # Test net output
    # dump_input = torch.rand((2, 5, 3, 736, 1280)).to(device)
    # dump_input = rearrange(dump_input, 'b l c h w -> (b l) c h w')
 ...
 # Resample
    ...
    Seg_loss = sstan_loss.lossStage1
    ...
    hrnet_params_ids = set([id(param) for param in hrnet_params])
    head1_param_ids = set([id(param) for param in head1_params])
    head2_param_ids = set([id(param) for param in head2_params])
    
    for param in model.module.parameters():
        if id(param) not in head1_param_ids and id(param) not in head2_param_ids and id(param) not in hrnet_params_ids:
            sstan_params.append(param)
            
    for param in hrnet_params:
        param.requires_grad = False
    for param in sstan_params:
    ...
    optimizer = optim.SGD([
        ...
        {'params': sstan_params, 'lr': config.TRAIN.LR},
    ...
    train_best_mIoU = 0
    best_mIoU = 0
    train_best_mpck15 = float('-inf')
    best_mpck15 = float('-inf')
    last_epoch = 0
        ...
        if epoch < config.TRAIN.STAGE1_EPOCH:
            ...
            train_total_loss, train_mIoU, train_IoU, train_accuracy, train_recall, train_precision, train_mdistance, train_mpck,\
                 = train(config, epoch, config.TRAIN.END_EPOCH, epoch_iters, config.TRAIN.LR, num_iters,trainloader, optimizer, model, 
                        Seg_loss, Landmark_loss, Landmark_loss2, L1_smooth2, writer_dict, device, stage, loss_weight)
        else:
            stage = 2
            if epoch == config.TRAIN.STAGE1_EPOCH:
                train_best_mIoU =0
