...
class PFSA(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=512, depth=1,
                 ...
                 drop_path_rate=0.1, hybrid_backbone=None, norm_layer=nn.LayerNorm, num_frames=5,
        ...
        # num_patches = 64 # the number of pixels in the input image, 1 pixel = 1 patch
        num_patches = 920
...
if __name__ == '__main__':
    model = VACSNet().cuda()
    dum_data = torch.randn(1, 11, 3, 736, 1280).cuda()
    preds = model(dum_data)