...
class MSTCN_Regression(nn.Module):
    def __init__(self, config):
        ...
        self.one_channel_layer = nn.Sequential(
            nn.Conv2d(
                ...
                stride=1,
                padding=0),
            BatchNorm2d(720, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
            nn.Conv2d(
                in_channels=720,
                out_channels=1,
        ...
        self.mstcn = mstcn.MultiStageModel(num_stages=2, num_layers=2, num_f_maps=4080, dim=58880, num_classes=4080)

        # For landmark head 
        self.landhead = nn.Linear(4080, 8)
...
class HRNet_MSTCN_Regression(nn.Module):
    def __init__(self, config):
        ...
        self.mstcn_regression = MSTCN_Regression(config)
            
            
    def forward(self, x):
        # x.shape = [3, 720, 184, 320]
        x= self.hrnet(x)
        cpts = self.mstcn_regression(x) 
        return cpts
