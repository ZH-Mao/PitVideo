...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.convlstm = convLSTM_multiimage.ConvLSTM(input_dim=720, hidden_dim=[720, 720], kernel_size=(3, 3), num_layers=2, batch_first=True, bias=True, return_all_layers=False)
    ...
    def forward(self, x):  # ([1, 3, 3, 720, 1296])
        x = x.view(-1, 3, 736, 1280) # ([3, 3, 720, 1296])
        x = self.conv1(x)
        ...
        # x = x.unsqueeze(0)
        x = x.view(-1, 3, 720, 184, 320)
        layer_output_list, _ = self.convlstm(x)
        
        
        # x = layer_output_list[0].squeeze(0)
        x = layer_output_list[0].view(-1, 720, 184, 320)
        # x.shape = [1, 3, 184, 320]
        seg = self.seghead(x)
