...
def train(config, epoch, num_epoch, epoch_iters, base_lr, num_iters,trainloader, optimizer, model, Seg_loss, Seg_loss2,
    ...
    for i_iter, batch in enumerate(tqdm(trainloader)):
        ...
        # else:
        #     cur_iters = (epoch-300)*epoch_iters
        #     lr = base_lr*((1-float(i_iter+cur_iters)/num_iters)**(0.9))
    ...
    msg = 'Epoch: [{}/{}], Time: {:.2f}, ' \
        'head1_lr: {:.6f}, head2_lr: {:.6f}, Train_total_Loss: {:.6f}, Train_seg_loss: {:.6f}, Train_bound_loss:{:.6f},Train_cpts_loss: {:.6f}, Train_presence_loss: {:.6f}'.format(
            epoch, num_epoch, batch_time.average(
            ), optimizer.param_groups[1]['lr'], optimizer.param_groups[2]['lr'],train_total_loss, train_segmentation_loss, train_bound_loss, train_centroid_loss, train_presence_loss
        )
    # msg = 'Epoch: [{}/{}], Time: {:.2f}, ' \
    ...
    writer.add_scalar('Learning_rate/head1', optimizer.param_groups[1]['lr'], global_steps)
    writer.add_scalar('Learning_rate/head2', optimizer.param_groups[2]['lr'], global_steps)
