...
def train(config, epoch, num_epoch, epoch_iters, base_lr, num_iters,trainloader, optimizer, model, Seg_loss, 
          Landmark_loss, Landmark_presence_loss, writer_dict, device, stage, loss_weight, scheduler):
    ...
    for i_iter, batch in enumerate(tqdm(trainloader)):
        ...
        seg_loss = Seg_loss(seg_out, labels[:, None, ...])
        ...
        presence_loss = presence_loss*loss_weight[2]
...
def validate(config, testloader, model, Seg_loss, Landmark_loss, Landmark_presence_loss, writer_dict, device, stage, loss_weight):
    model.eval()
    ave_loss = AverageMeter()
    ave_seg_loss = AverageMeter()
    # ave_bound_loss = AverageMeter()
    ...
    with torch.no_grad():
        for _, batch in enumerate(tqdm(testloader)):
            ...
            seg_loss = Seg_loss(pred, label[:, None, ...])
...
def test(config, testloader, model,
    ...
    with torch.no_grad():
        for _, batch in enumerate(tqdm(testloader)):
            ...
            if sv_pred:
                sv_path = os.path.join(sv_dir, 'test_results4')
