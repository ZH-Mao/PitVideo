...
def compute_dtm(img_gt: Tensor, pred: Tensor) -> Tensor:
    ...
    for b in range(out_shape[0]):  # batch size
        for c in range(1, out_shape[1]):  # default 0 channel is background
            ...
            if posmask.any():
                posdis = distance(posmask.cpu())
                negdis = distance(negmask.cpu())  # zhehua
...
def hd_loss(seg_soft: Tensor,
    ...
    seg = torch.argmax(seg_soft, dim=1, keepdim=False).long()
...
class HuasdorffDisstanceLoss(nn.Module):
    ...
    def forward(self,
        ...
        with torch.no_grad():
            gt_dtm = compute_dtm(target, pred_soft)
            gt_dtm = gt_dtm.float()
            seg_dtm2 = compute_dtm(
                pred_soft.argmax(dim=1, keepdim=False), pred_soft)
