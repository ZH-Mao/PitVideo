...
def train(config, epoch, num_epoch, epoch_iters, base_lr, num_iters,trainloader, optimizer, model, Seg_loss, Seg_loss2,
    ...
    for i_iter, batch in enumerate(tqdm(trainloader)):
        ...
        if stage == 1:
            ...
            # cpts_loss2 = Landmark_loss2(torch.reshape(cpts_out, (cpts_out.size(0),-1)) , torch.reshape(cpts_gt, (cpts_gt.size(0),-1)))
        ...
        squared_distance[:, :, 0] = norm_squared_distance[:, :, 0]*(1296**2)
