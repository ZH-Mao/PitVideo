...
def validate(config, testloader, model, Seg_loss, Landmark_loss, Landmark_loss2, writer_dict, device, stage, loss_weight):
    ...
    with torch.no_grad():
        for _, batch in enumerate(tqdm(testloader)):
            ...
            label, cpts_gt, cpts_presence = label[:,-1,:,:], cpts_gt[:,-1,:,:], cpts_presence[:,-1,:,:]
            size = label.size()
            ...
            seg_pre = seg_pre.unsqueeze(0)[:,-1,:,:,:] # here is a bug, if batch size is greater than 1
            cpts_pre = cpts_pre.unsqueeze(0)[:,-1]
