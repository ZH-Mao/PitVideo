...
def train(config, epoch, num_epoch,trainloader, optimizer, model, Seg_loss, 
          writer_dict, device, scheduler):
    ...
    #     )
    msg = 'Epoch: [{}/{}], Time: {:.2f}, ' \
        'lr: {:.6f}, Train_total_Loss: {:.6f}, Train_seg_loss: {:.6f}'.format(
            epoch, num_epoch, batch_time.average(
            ), optimizer.param_groups[0]['lr'], train_total_loss, train_segmentation_loss
        )
    logging.info(msg)
    ...
    return train_total_loss, train_mIoU, train_IoU, train_accuracy, train_recall, train_precision


def validate(config, testloader, model, Seg_loss, writer_dict, device):
    model.eval()
    ave_loss = AverageMeter()
    ave_seg_loss = AverageMeter()
    # ave_bound_loss = AverageMeter()
    ...
    msg = 'Valid_total_Loss: {:.6f}, Valid_seg_loss: {:.6f}'.format(
            val_total_loss, val_segmentation_loss)
    logging.info(msg)
    
    return val_total_loss, mean_IoU, IoU_array, accuracy, recall, precision
