...
class PitDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple:      
        ...
        mask4 = cv2.imread(os.path.join(self.mask_root, self.video_frames.iloc[frameID_in_video[0]-4, 1]), cv2.IMREAD_GRAYSCALE)/127
        ...
        mask3 = cv2.imread(os.path.join(self.mask_root, self.video_frames.iloc[frameID_in_video[0]-3, 1]), cv2.IMREAD_GRAYSCALE)/127
        ...
        mask2 = cv2.imread(os.path.join(self.mask_root, self.video_frames.iloc[frameID_in_video[0]-2, 1]), cv2.IMREAD_GRAYSCALE)/127
        ...
        mask1 = cv2.imread(os.path.join(self.mask_root, self.video_frames.iloc[frameID_in_video[0]-1, 1]), cv2.IMREAD_GRAYSCALE)/127
        ...
        mask = cv2.imread(os.path.join(self.mask_root, self.video_frames.iloc[frameID_in_video[0], 1]), cv2.IMREAD_GRAYSCALE)/127
        ...
        if self.is_train:
            ...
            #     image2=image2, image1=image1,image=image, mask2=mask2, mask1=mask1, mask=mask, keypoints2=cpts2, keypoints1=cpts1, keypoints=cpts)
            # image2, image1, image = sample['image2'], sample['image1'], sample['image']
            # mask2, mask1, mask = sample['mask2'], sample['mask1'], sample['mask']
            # cpts2, cpts1, cpts = sample['keypoints2'], sample['keypoints1'], sample['keypoints']
            # frames_sequence = torch.stack([torch.from_numpy(image2), torch.from_numpy(image1), torch.from_numpy(image)], dim=0)
            # mask_sequence = torch.stack([torch.from_numpy(mask2), torch.from_numpy(mask1), torch.from_numpy(mask)], dim=0)
            # cpts_sequence = torch.stack([torch.from_numpy(np.array(cpts2)), torch.from_numpy(np.array(cpts1)), torch.from_numpy(np.array(cpts))], dim=0)
            sample = self.transform_val(
                image4=image4, image3=image3, image2=image2, image1=image1,image=image, 
                mask4=mask4, mask3=mask3, mask2=mask2, mask1=mask1, mask=mask, 
                keypoints4=cpts4, keypoints3=cpts3, keypoints2=cpts2, keypoints1=cpts1, keypoints=cpts)
